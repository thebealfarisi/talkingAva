...
async function callFunctionsSequentially() {
  ...
  try {
    ...
    // Call startAndDisplaySession next
    await startAndDisplaySession();
    console.log("startAndDisplaySession completed");
...
async function startAndDisplaySession() {
  if (!sessionInfo) {
    uconsole.log('Please create a connection first');
    return;
  }
